    ...
    class Composer : Game
    {
        ...
        public static void PlayNotes(List<songNotes> songNotes)
        {
            foreach (songNotes songNote in songNotes)
            {
                if (songNote.note != 0)
                {
                    Console.Beep(songNote.note, songNote.duration);
                }
                else
                {
                    Thread.Sleep(songNote.duration);
                }
            }
        }

        public static int NoteToFreq(string note)
        {
            if (note == "C4")
                return 261;
            if (note == "C#" || note == "Db")
        ...
        public static int DurationToTime(string note)
        {
            if (note == "1/4")
                return 1600 / 4;
            if (note == "1/2")
        ...
        private static void SerialzeJSON(List<songNotes> currentSongNotes)
        {
            ...
            File.WriteAllText(@"c:\currentSongNotes.json", JsonConvert.SerializeObject(currentSongNotes));

            // serialize JSON directly to a file
            using (StreamWriter file = File.CreateText(@"c:\currentSongNotes.json"))
            {
        ...
        private static void DeserialzeJSON()
        {
            // read file into a string and deserialize JSON to a type
            //List<songNotes> movie1 = JsonConvert.DeserializeObject<List<songNotes>>(File.ReadAllText(@"c:\currentSongNotes.json"));

            // deserialize JSON directly from a file
            using (StreamReader file = File.OpenText(@"c:\currentSongNotes.json"))
            {
                JsonSerializer serializer = new JsonSerializer();
                List<songNotes> movie2 = (List<songNotes>)serializer.Deserialize(file, typeof(List<songNotes>));
            }
        }*/
